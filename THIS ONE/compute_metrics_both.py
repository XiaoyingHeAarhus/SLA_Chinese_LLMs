...
# 遵循原文第4.3节的方法，做了两处有据可查的改动：
#   1. EuroBERT → hfl/chinese-roberta-wwm-ext（语言适配）
#   2. 同时计算两个 PLL_metric 变体以便对比：
#      - 'within_word_l2r'：Kauf & Tily (2023) 推荐，更适合全词掩码模型
#      - 'original'       ：minicons 默认值，与原文行为一致
#
...
# BERT_MAX_CHARS 是根据 BERT 512 token 上限反推的字符数安全阈值。
# 中文每个字符基本对应1个 token，留一些余量给特殊符号（[CLS]、[SEP]等）。
BERT_MAX_CHARS = 490

def split_to_bert_limit(sentences: list[str]) -> list[str]:
    """
    为什么需要这个函数：
    BERT 类模型（包括 chinese-roberta-wwm-ext）有512 token的硬性上限
    （hard limit）。高HSK级别的消息有时非常长，直接超出限制会报
    "expanded size must match existing size" 错误。

    为什么用细切而不是截断（truncation）：
    高HSK级别的长句恰恰是我们最想测量的数据。如果截断，就系统性地
    丢失了高级别文本的后半段信息，会低估（underestimate）高级别的惊异度。
    细切保留所有内容，只是换了更细的粒度。

    怎么做：
    对每个超过 BERT_MAX_CHARS 的句子，按次级标点（secondary punctuation）
    进一步切分：，（逗号）、（顿号）；（分号）：（冒号）——（破折号）。
    如果细切后还有超长片段（极少见），继续按空格或强制切分，
    确保送入模型的每个片段都在安全长度以内。
    """
    result = []
    for s in sentences:
        if len(s) <= BERT_MAX_CHARS:
            result.append(s)  # 长度正常，直接保留
        else:
            # 按次级标点（secondary punctuation）细切
            # [，、；：——]+ 表示一个或多个这类标点（连续标点合并处理）
            sub = re.split(r'[，、；：——]+', s)
            sub = [x.strip() for x in sub if len(x.strip()) > 1]

            for fragment in sub:
                if len(fragment) <= BERT_MAX_CHARS:
                    result.append(fragment)  # 细切后长度正常
                else:
                    # 极端情况：细切后仍然超长（比如一个超长无标点段落）
                    # 按 BERT_MAX_CHARS 强制分段，不丢弃任何内容
                    # range(0, len, step) 生成切分起始位置
                    for i in range(0, len(fragment), BERT_MAX_CHARS):
                        chunk = fragment[i:i + BERT_MAX_CHARS]
                        if len(chunk) > 1:
                            result.append(chunk)
    return result


def compute_message_surprisal(text: str, model, pll_metric: str) -> float:
    """
    为什么需要这个指标：
    ...
    怎么做（严格遵循原文第4.3节）：
      1. 把消息切分成句子
      2. 对超长句子做细切，避免超过 BERT 的 512 token 上限
      3. 对每个句子计算 token 级别的平均惊异度
         （= 负对数概率，按 token 数量归一化）
      4. 对所有句子的惊异度取平均 → 消息惊异度（Message Surprisal）

    pll_metric 参数控制伪对数似然（pseudo-log-likelihood）的计算方式：
      'within_word_l2r'：Kauf & Tily (2023) 推荐，专为全词掩码模型设计，
                         在预测一个词的各字符时，利用同词内左侧已预测字符
                         作为额外条件，更准确地捕捉词内部的依赖关系。
                         适合 hfl/chinese-roberta-wwm-ext（全词掩码模型）。
      'original'       ：minicons 的默认值，与原文 EuroBERT 的行为一致，
                         逐 token 独立 mask 和预测，不考虑词内部结构。
    """
    import torch  # 在函数内导入，只在需要时才加载
    ...
    # 把超过BERT上限的句子细切成更短的片段，避免 "expanded size" 报错
    sentences = split_to_bert_limit(sentences)
    ...
    try:
        ...
        surprisals = model.sequence_score(
            ...
            PLL_metric=pll_metric  # 由调用方传入，支持对比不同方法
        )
        ...
        if "out of memory" in str(e).lower():
            # 显存不足（OOM, Out Of Memory）时：
            # 清空 GPU 显存缓存（cache）后改用 CPU 重试
            print(f"  GPU OOM，改用CPU重试（{pll_metric}）：{text[:40]!r}")
            torch.cuda.empty_cache()  # 释放 PyTorch 占用但未使用的显存
            try:
                model.model.to("cpu")
                surprisals = model.sequence_score(
                    sentences,
            ...
            print(f"  警告：惊异度计算失败（{pll_metric}），文本片段：{text[:50]!r} — {e}")
            return np.nan
...
def process_file(
    ...
    for msg in tutor_messages:
        ...
        # 同时计算两个 PLL_metric 变体，方便论文里对比：
        # surprisal_wwm：within_word_l2r，专为全词掩码模型设计（我们的主要方法）
        # surprisal_orig：original，minicons默认值，与原文EuroBERT行为一致
        surprisal_wwm  = compute_message_surprisal(content, surprisal_model, 'within_word_l2r')
        surprisal_orig = compute_message_surprisal(content, surprisal_model, 'original')
        ...
        rows.append({
            ...
            "surprisal_wwm":          surprisal_wwm,         # 指标4a：within_word_l2r（主要方法）
            "surprisal_original":     surprisal_orig,        # 指标4b：original（与原文一致）
        })
...
def main():
    ...
    # .ipynb_checkpoints 是 Jupyter Notebook 自动生成的隐藏缓存文件夹，
    # 不是真实数据，必须过滤掉，否则路径层级（parts）会读错，
    # 导致 v4.0 被误识别为模型名，.ipynb_checkpoints 被误识别为HSK级别
    json_files = sorted(
        f for f in data_dir.rglob("*.json")
        if ".ipynb_checkpoints" not in f.parts
    )
    print(f"找到 {len(json_files)} 个JSON文件（已排除 .ipynb_checkpoints）。")
    ...
    print(
        df.groupby(["model", "level"])[
            ["sentence_length", "text_length", "mdd", "surprisal_wwm", "surprisal_original"]
        ].mean().round(3)
    )
